...
In this setting, we consider the target model as a trained GNN for node classification tasks.
The adversary aims to infer whether a user's node in the graph was used in training the target model $f()$.
In a blackbox setting, the adversary has only access to the model outputs $f(x;W)$ for a given input $x$.
...
\begin{figure}[!htb]
    ...
    \includegraphics[width=0.48\linewidth]{figures/BBMIA/citeseer_hist.pdf}
...
%\vspace{-2mm}
    ...
    \includegraphics[width=0.48\linewidth]{figures/BBMIA/cora_hist.pdf}
...
%\vspace{-2mm}
    \subfigure[Pubmed]{
    \label{fig:mem_soft_label}
    \includegraphics[width=0.48\linewidth]{figures/BBMIA/pubmed_hist.pdf}
...
Predicting with higher confidence in seen $G_{train}$ nodes compared to unseen test nodes is referred to as overfitting.
This difference in the output prediction confidence directly results from a distinguishable output distribution between the train and test data indicated by a non-overlapping region between distributions (Figure~\ref{fig:NIAcause}, right).
...
%There have been theoretical results (http://proceedings.mlr.press/v97/sablayrolles19a/sablayrolles19a.pdf) which have shown that whitebox attacks do not perform any better than blackbox attacks. These have been empirically verified as well (https://arxiv.org/abs/1906.06589)(https://arxiv.org/abs/2003.10595) where the blackbox membership inference attacks do better than the whitebox attacks. We should view blackbox and whitebox as two possible threat model and attacks settings instead of considering that adversary will have more knowledge in whitebox than blackbox and hence should perform better. Maybe we can change the attack titles from blackbox to "prediction attack" and whitebox to "embedding attack".
...
This is a practical adversary assumption in the case of federated learning where intermediate computations can be observed~\cite{whitebox,collabinf}.
...
%\noindent\textbf{Attack Methodology.}
The attack methodology is unsupervised and the adversary has no supervised labels to map the intermediate embeddings to a membership value.
...
In order to distinguish the clusters, the adversary requires to know the prior mapping of a small number of members and non-members.
